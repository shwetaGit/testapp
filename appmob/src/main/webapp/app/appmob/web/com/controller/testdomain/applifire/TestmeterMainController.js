Ext.define('Appmob.appmob.web.com.controller.testdomain.applifire.TestmeterMainController', {
     extend: 'Appmob.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.TestmeterMainController',
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#TestmeterForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#TestmeterGrid');
               var tree = this.view.down('#TestmeterTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#TestmeterForm');
          var grid = this.view.down('#TestmeterGrid');
          var tree = this.view.down('#TestmeterTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#TestmeterGrid');
               ...
               var formPanel = this.getView().up().down('#TestmeterForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var data = this.findRecordById(this.view.restURL, jsonData);
          var treePanel = this.view.up().up().down('#TestmeterTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#TestmeterForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#TestmeterGrid').store;
          gridStore.removeAll();
     ...
     setGridConfig: function() {
          ...
          var gridStore = Ext.create('Ext.data.Store', {
               model: 'Appmob.appmob.shared.com.model.testdomain.applifire.TestmeterModel',
               autoLoad: {
                    params: {
               ...
               listeners: {
                    load: function(storeObj, records, successful, eOpts) {
                         var tree = currentObject.view.down('#TestmeterTree');
